BEGIN {
  ...
  track_hidden=0;
  track_stack_name_deps=0;
  sraw=0;
  swar=0;
  swaw=0;
  sctl=0;
}
file=="source" && (min <= FNR && (max==0 || FNR <= max))  {
  srcmax=FNR;
  numstr = FNR "";
  prstr = "";
...
(max==0 || $4 <= max) {
  ...
  if( (!track_hidden && tag ~ /H/) || ( (!sraw || tag !~ /T/) && ( ( (!swar || tag !~ /A/ ) && (!swaw || tag !~ /O/)) || (!track_stack_name_deps && tag ~ /S/)) && (!sctl || tag !~ /C/))) {
        next;
  }
  ...
  if( srcLine != dstLine ) {
    if( edges[srcLine,dstLine] < depty ) {
      edges[srcLine,dstLine] = depty;
       # printf "%2d -> %2d\n", srcLine, dstLine > "/dev/stderr";
    }
    social[srcLine] = 1;
    social[dstLine] = 1;
  }
...
#        }
  ...
  printf "\\begin{picture}(420,%d)(%d,%d)\n\n", 
          10*(max-min+1), leftmargin, -10*(max-min+1);
 
  for( line=min; line<=max; line++ ) {

    printf "\\put(%d,%d){\\makebox(15,10)[r]{\\it %d:}}%%\n",
      leftmargin, -10*(line-min+1), line;

    printf "\\put(%d,%d){\\makebox(100,10)[l]{%s}}\n", 
           0, -10*(line-min+1), src[line];

    if( line in social ) {
      printf "{\\color{black} \\dottedline{3}(%d,%d)(0,%d)}\n", 
        -10, 
        -10*(line-min+1)+5, -10*(line-min+1)+5
    }
  }
  printf "\n";
  ...
  for( line in social ) {
    printf "\\put(%d,%d){\\color{black}\\circle*{2}}\n", 
           10*placement[line]-ncoff, -10*(line-min+1)+5;
  }
  ...
  for( srcL=1; srcL <= srcmax; srcL++ ) {
    for( dstL=1; dstL <= srcmax; dstL++ ) {
      if( (srcL,dstL) in edges ) {
        ...
        Ystart = -depcolw*(srcL-min+1) + depcolw/2 + Ytwoff + YoffN;
