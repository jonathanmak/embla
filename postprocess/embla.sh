...
LENGTHS_FILE_PLOOP=$BASENAME.pllengths
...
TASK_SIZE_FILE_PLOOP=$BASENAME.pltasks

if [ ! $HIDDEN_FUNC_FILE ]
then HIDDEN_FUNC_FILE=$EMBLA_BIN/embla.hidden-funcs
fi

if [ ! $N_TRACE_RECS ]
then N_TRACE_RECS=80000000
fi
...
echo "$EMBLA_BIN/bin/valgrind --tool=embla --hidden-func-file=$HIDDEN_FUNC_FILE --n-trace-recs=$N_TRACE_RECS --loop-file=$LOOP_FILE --trace-file=$TRACE_FILE_LOOP --edge-file=$EDGES_FILE_LOOP $CLIENT_PROG 1>&2"
eval "$EMBLA_BIN/bin/valgrind --tool=embla --hidden-func-file=$HIDDEN_FUNC_FILE --n-trace-recs=$N_TRACE_RECS --loop-file=$LOOP_FILE --trace-file=$TRACE_FILE_LOOP --edge-file=$EDGES_FILE_LOOP $CLIENT_PROG 1>&2"
...
echo "$EMBLA_BIN/bin/valgrind --tool=embla --hidden-func-file=$HIDDEN_FUNC_FILE --n-trace-recs=$N_TRACE_RECS --loop-file=$PLOOP_FILE --trace-file=$TRACE_FILE_PLOOP --edge-file=$EDGES_FILE_PLOOP --lengths-file=$LENGTHS_FILE_PLOOP $CLIENT_PROG 1>&2"
eval "$EMBLA_BIN/bin/valgrind --tool=embla --hidden-func-file=$HIDDEN_FUNC_FILE --n-trace-recs=$N_TRACE_RECS --loop-file=$PLOOP_FILE --trace-file=$TRACE_FILE_PLOOP --edge-file=$EDGES_FILE_PLOOP --lengths-file=$LENGTHS_FILE_PLOOP $CLIENT_PROG 1>&2"
...
# Work out task sizes
# $GRAN_ANALYSIS --sraw --swar --swaw --sctl --dep-file $DEPS_FILE_PLOOP --edge-file $EDGES_FILE_PLOOP --lengths-file $LENGTHS_FILE_PLOOP >$TASK_SIZE_FILE_PLOOP
