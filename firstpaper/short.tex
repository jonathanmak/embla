...
The endpoints of the dependency arrow discussed above are parts of
...
only the tree dependency between the nodes {\it B} and {\it D} and the NCA of
...
a flow dependency (RAW). If it is a write, we use the previous write to 
...
anti dependency edges explicitly. When that write has been processed 
...
\tt nfib & 22 & 7 & 32M & 14K & 6.7 & 32M & 14K & 6.9 \\
\tt qs & 79 & 83 & 82M & 39M & 24.7 & 82M & 35M & 22.7 \\
\tt mpeg & 6053 & 3330 & & & & 3.2G & 109M & 626.5 \\ \hline
\end{tabular}
\end{center}
...
expected. Some results are reported in Table~\ref{trex}. It is interesting
...
locations are (the RSz columns). We also see that the number of instruction 
...
in the recursive divide-and-conquer programs quicksort ({\tt qs}) and
fibonnaci ({\tt nfib}) although the former is an in-place version coded 
