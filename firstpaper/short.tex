...
% computational cost, especially when analyzing large programs.
...
% To be useful to the programmer, the instruction level dependences detected 
% by Embla must be mapped to source level. As discussed above, the dependence
% that Embla reports is not always in the same function as the dependence endpoints
% Embla is designed to help programmers 
% rewrite source code, so we need to project 
% the dependences discovered in the dynamic instruction stream onto the 
...
% procedure calls, we need to reflect the dependences we find to that level 
% as well. That is, we always want to know the dependences between different 
